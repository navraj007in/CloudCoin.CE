...
namespace Founders
{
    ...
    public class Detector
    {
		...
		private void UpdateStatus(string status,String SecondStatus, int percentage = 0)
		{
			// Make sure someone is listening to event
			if (OnUpdateStatus == null) return;

            ProgressEventArgs args = new ProgressEventArgs(status,SecondStatus, percentage);
			OnUpdateStatus(this, args);
		}
			...
			for (int j = 0; j < suspectFileNames.Length; j++)
			{
				try
				{
					...
					else
					{
                        ...
                        string coinStatus = cu.getFolder();
                        if (coinStatus.ToLower().Equals("bank"))
                            coinStatus = "Authentic";
                        UpdateStatus("","Coin detected as " + coinStatus);
